<script>
  ...
  async function handleDelete() {
    if (confirm(`Vorlage "${template.name}" deaktivieren?`)) {
...
{#if template}
  <div class="page">
    ...
    <div class="meta">
      ...
      <span class="meta-item">{inspectionCount} Pruefung(en) durchgefuehrt</span>
    </div>
    ...
    <h2>Pruefpunkte ({items.length})</h2>
    {#if items.length === 0}
      <p class="empty">Keine Pruefpunkte definiert.</p>
    {:else}
      <table>
        <thead>
          <tr><th>Nr.</th><th>Pruefpunkt</th><th>Hinweis</th><th>Pflicht</th></tr>
        </thead>
        <tbody>
    ...
    <button class="btn-secondary" onclick={() => currentView.set('templates')}>Zurueck</button>
  </div>
{/if}
