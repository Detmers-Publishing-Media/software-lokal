...
describe('appendEvent', () => {
  ...
  it('speichert vollstaendigen Snapshot als JSON', async () => {
    const snapshot = { id: 1, vorname: 'Max', nachname: 'Mustermann' };
    const event = await appendEvent('KundeAngelegt', snapshot);
    const parsed = JSON.parse(event.data);
    assert.deepEqual(parsed, snapshot);
  });

  it('Event-Typ wird korrekt gespeichert', async () => {
    const e1 = await appendEvent('KundeAngelegt', { id: 1 });
    const e2 = await appendEvent('KundeGeloescht', { id: 1 });
    assert.equal(e1.type, 'KundeAngelegt');
    assert.equal(e2.type, 'KundeGeloescht');
  });
  ...
  it('alle Berater-Lokal Event-Typen funktionieren', async () => {
    const types = [
      'KundeAngelegt', 'KundeGeaendert', 'KundeGeloescht',
      'KindAngelegt', 'KindGeaendert', 'KindGeloescht',
      'EinnahmeAngelegt', 'EinnahmeGeaendert', 'EinnahmeGeloescht',
      'AusgabeAngelegt', 'AusgabeGeaendert', 'AusgabeGeloescht',
      'PoliceAngelegt', 'PoliceGeaendert', 'PoliceGeloescht',
      'VermoegenAngelegt', 'VermoegenGeaendert', 'VermoegenGeloescht',
      'VerbindlichkeitAngelegt', 'VerbindlichkeitGeaendert', 'VerbindlichkeitGeloescht',
      'AltersvorsorgeAngelegt', 'AltersvorsorgeGeaendert', 'AltersvorsorgeGeloescht',
      'ProfilGespeichert', 'AppGestartet',
    ];
    for (const type of types) {
      await appendEvent(type, { test: true });
    }
