...
describe('verifyChain (Integritaet)', () => {
  ...
  it('erkennt unveraenderte Kette als gueltig', async () => {
    await insertEvent(db, 'KundeAngelegt', { id: 1, vorname: 'Max' });
    await insertEvent(db, 'EinnahmeAngelegt', { id: 1, betrag: 3500 });
    await insertEvent(db, 'PoliceAngelegt', { id: 1, sparte: 'BU' });
  ...
  it('erkennt geaendertes data-Feld', async () => {
    await insertEvent(db, 'KundeAngelegt', { id: 1, vorname: 'Max' });
    await insertEvent(db, 'KundeGeaendert', { id: 1, vorname: 'Max' });

    // Tamper with data of event 2
    db.prepare("UPDATE events SET data = '{\"id\":1,\"vorname\":\"MANIPULIERT\"}' WHERE id = 2").run();
